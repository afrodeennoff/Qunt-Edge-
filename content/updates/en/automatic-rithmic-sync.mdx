---
title: Automatic trade sync from Rithmic
description: Connect your Rithmic account for automatic hourly synchronization of all your trades.
date: 2025-01-06
status: completed
completedDate: 2025-01-06
image: '/posts/rithmic/rithmic.png'
---

# Automatic trade sync from Rithmic

No more manual CSV exports. Connect your Rithmic account and your trades sync automatically every hour.

<iframe
  className="w-full aspect-video rounded-lg"
  src="https://www.youtube.com/embed/HdaWk1y1zK8"
  title="Automatic Rithmic Import Feature Demo"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
/>

## How it works

Enter your Rithmic credentials in the account settings. QuntEdge connects to Rithmic's API and fetches your trades automatically. New trades appear in your dashboard within an hour of execution.

## Key capabilities

- **Hourly sync**: Trades are fetched every hour automatically
- **Historical import**: Pull up to 400 days of past trades
- **Multi-account support**: Connect multiple Rithmic accounts
- **Real-time feedback**: See sync status and progress
- **Secure connection**: Credentials are encrypted and stored safely

## Getting started

1. Go to account settings
2. Select "Connect Rithmic Account"
3. Enter your Rithmic credentials
4. Confirm the connection

Your trades will sync automatically from that point forward.
